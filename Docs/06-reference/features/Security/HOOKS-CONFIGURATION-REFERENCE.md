...
| Validator | Purpose |
|-----------|---------|
| `token-validator.js` | Validates authentication token |
| `token-validator.js` | Initializes security state |
...
|-----------|---------|-----------|
| `prompt-injection.js` | Detects embedded instructions | Injection patterns found |
| `jailbreak.js` | Detects jailbreak attempts | Jailbreak patterns matched |
...
| `supply-chain.js` | SHA256/GPG integrity check | Skill file tampered |
| `rate-limiter.js` | DoS protection | Rate limit exceeded |
...
|-----------|---------|-----------|
| `rate-limiter.js` | DoS protection | Rate limit exceeded |
| `recursion-guard.js` | Prevents infinite loops | Depth/recursion limit hit |

#### Bash Tool

| Validator | Purpose | Blocks If |
|-----------|---------|-----------|
| `bash-safety.js` | Dangerous command detection | rm -rf, sudo, etc. |
| `production.js` | Production targeting | Prod environment detected |
| `outside-repo.js` | Repository boundary | Path outside repo |
| `plugin-permissions.js` | Capability check | Plugin lacks shell permission |
| `rate-limiter.js` | DoS protection | Rate limit exceeded |
| `resource-limits.js` | Memory/CPU limits | Resources exceeded |

#### Write Tool

| Validator | Purpose | Blocks If |
|-----------|---------|-----------|
| `secret.js` | Credential detection | API keys, tokens found |
| `env-protection.js` | Sensitive file protection | Writing to .env files |
| `outside-repo.js` | Repository boundary | Path outside repo |
| `pii.js` | PII detection | SSN, credit cards found |
| `prompt-injection.js` | Content validation | Injection in content |
| `plugin-permissions.js` | Capability check | Plugin lacks write permission |
| `rate-limiter.js` | DoS protection | Rate limit exceeded |
...
|-----------|---------|-----------|
| `secret.js` | Credential detection | API keys, tokens found |
| `env-protection.js` | Sensitive file protection | Editing .env files |
| `outside-repo.js` | Repository boundary | Path outside repo |
| `pii.js` | PII detection | SSN, credit cards found |
| `prompt-injection.js` | Content validation | Injection in content |
| `plugin-permissions.js` | Capability check | Plugin lacks write permission |
...
|-----------|---------|-----------|
| `outside-repo.js` | Repository boundary | Path outside repo |
| `prompt-injection.js` | Content scanning | Malicious content detected |
| `plugin-permissions.js` | Capability check | Plugin lacks read permission |
| `rate-limiter.js` | DoS protection | Rate limit exceeded |
| `recursion-guard.js` | Depth limits | Too many recursive reads |

#### Glob Tool

| Validator | Purpose | Blocks If |
|-----------|---------|-----------|
| `outside-repo.js` | Repository boundary | Pattern outside repo |
| `rate-limiter.js` | DoS protection | Rate limit exceeded |
| `recursion-guard.js` | Depth limits | Deep directory traversal |

#### Grep Tool

| Validator | Purpose | Blocks If |
|-----------|---------|-----------|
| `outside-repo.js` | Repository boundary | Search path outside repo |
| `rate-limiter.js` | DoS protection | Rate limit exceeded |
...
| Validator | Purpose | Blocks If |
|-----------|---------|-----------|
| `plugin-permissions.js` | Capability check | Plugin lacks network permission |
| `rate-limiter.js` | DoS protection | Rate limit exceeded |
...
### Resource Limits (resource-limits.js)
...
### Recursion Limits (recursion-guard.js)
...
- **rate-limiter.js**: Critical config files exempt
- **outside-repo.js**: System paths allowed for reads
- **pii.js**: Test files exempt
...
4. Log to security audit log via `security-common.js`
