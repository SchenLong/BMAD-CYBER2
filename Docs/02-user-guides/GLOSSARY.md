...
Security model where components are granted specific capabilities rather than broad permissions. Used by `plugin-permissions.js`.
...
### Compact Agent
A compressed version of an agent persona (~200 tokens vs ~5,500 tokens for full) containing essential role, voice, and core principles. Used in Tier 1 context loading. Located in `_bmad/_compact/agents/`.

### CONCURA
Context Curation Efficiency Initiative. The project that implemented tiered context loading, achieving 8.75x average token reduction while preserving 8.6/10 quality score.

### Context Escalation
Automatic upgrade from a lower context tier to a higher one when the system detects the user's query requires more detailed information.

### Context Tier
One of three levels of context loading: Tier 0 (discovery, ~500 tokens), Tier 1 (standard, ~2,000 tokens), or Tier 2 (full, ~10,000 tokens).
...
### Lazy Loading
Context loading strategy that loads additional context only when needed, rather than eagerly loading everything upfront. Core principle of the CONCURA system.
...
### Micro-Manifest
Ultra-compact CSV files containing 10-word summaries of agents/workflows with routing tags. Used for Tier 0 discovery. Files: `micro-agent-manifest.csv`, `micro-workflow-manifest.csv`.
...
Attack where malicious instructions are embedded in prompts. Mitigated by `jailbreak.js`.
...
Vulnerabilities from untrusted plugins or dependencies. Mitigated by `supply-chain.js`.

### OWASP LLM07 (Insecure Plugin Design)
Security risks from poorly designed plugins. Mitigated by `plugin-permissions.js`.

### OWASP LLM09 (Overreliance)
Risk from excessive trust in AI outputs. Mitigated by `confidence-tracker.js`.
...
### Rate Limiting
Control mechanism that restricts the frequency of operations. Implemented via sliding window algorithm in `rate-limiter.js`.
...
System that loads context progressively (Tier 0 → Tier 1 → Tier 2) based on user intent, reducing token consumption by up to 8.75x.

### Token
Encrypted credential containing user identity and permissions.

### Token Efficiency
Ratio of useful work accomplished to tokens consumed. CONCURA achieves 8.75x improvement in token efficiency.
...
### Validator
Node.js script that validates operations against security rules.
...
| BMGD | BMAD Game Development |
| BMM | BMAD Methodology |
| CONCURA | Context Curation Efficiency (Initiative) |
| C2 | Command and Control |
| CIS | Creative Innovation Suite |
| DAN | Do Anything Now |
