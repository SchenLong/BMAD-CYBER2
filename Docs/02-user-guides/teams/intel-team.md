...
```mermaid
graph TD
    A[Intelligence Requirement] --> B[Collection Strategy]
    B --> C[OSINT Collection]
    B --> D[HUMINT Collection]
    B --> E[SIGINT Collection]
    B --> F[GEOINT Collection]
    B --> G[TECHINT Collection]

    C --> H[All-Source Fusion]
    D --> H
    E --> H
    F --> H
    G --> H

    H --> I[Intelligence Product]
    I --> J[Strategic Decision]
```
