---
stepsCompleted: []
createdDate: {date}
workflow: ma-due-diligence
status: draft
targetName: ""
dealType: ""
---

# M&A Due Diligence: {target}

## Executive Summary

[One paragraph synthesis of the deal thesis, key findings, and recommendation]

---

## 1. Deal Thesis

### Transaction Overview
**Deal Type:** [Acquisition/Merger/Investment/JV/Divestiture]
**Target:** [Company name]
**Transaction Structure:** [Cash/Stock/Mixed]
**Indicative Valuation:** [Range]

### Strategic Rationale
[Clear articulation of why this deal makes sense]

### Target Profile
| Attribute | Details |
|-----------|---------|
| Industry | |
| Revenue | |
| Employees | |
| Ownership | |
| Market Position | |

### Deal Parameters
- Timeline:
- Valuation Range:
- Financing:
- Key Conditions:

### Success Criteria
[How will we measure deal success?]

---

## 2. Strategic Fit Analysis

### Market Position
[Analysis of target's market position and competitive dynamics]

### Strategic Alignment Matrix
| Dimension | Target Strength | Your Need | Fit Score (1-5) |
|-----------|-----------------|-----------|-----------------|
| Market access | | | |
| Technology/IP | | | |
| Talent | | | |
| Customer base | | | |
| Product portfolio | | | |
| Geographic reach | | | |

**Overall Strategic Fit:** [X/5]

### Competitive Impact
[How this deal changes the competitive landscape]

### Strategic Synergies
**Revenue Synergies:**
-

**Strategic Synergies:**
-

### Strategic Risks
-

### Sun's Strategic View
"[Master strategist perspective]"

---

## 3. Financial Assessment

### Financial Health Summary
[Key findings on target's financial health]

### Historical Performance
| Metric | Year -2 | Year -1 | Current | Trend |
|--------|---------|---------|---------|-------|
| Revenue | | | | |
| Revenue Growth | | | | |
| EBITDA | | | | |
| EBITDA Margin | | | | |
| Free Cash Flow | | | | |

### Valuation Analysis
| Methodology | Low | Mid | High |
|-------------|-----|-----|------|
| Comparable Companies | | | |
| Precedent Transactions | | | |
| DCF | | | |
| **Blended Range** | | | |

### Synergy Quantification

**Cost Synergies:**
| Category | Value | Timing | Confidence |
|----------|-------|--------|------------|
| | | | |
| **Total Cost** | | | |

**Revenue Synergies:**
| Category | Value | Timing | Confidence |
|----------|-------|--------|------------|
| | | | |
| **Total Revenue** | | | |

**Net Synergy Value:** $X (after $Y implementation costs)

### Deal Economics
- Accretion/Dilution:
- Expected IRR:
- Payback Period:

### Financial Risks
-

### Augustus's Evidence-Based View
"[Policy analyst perspective]"

---

## 4. Operational Due Diligence

### Operations Assessment
| Dimension | Target State | Gap | Integration Complexity |
|-----------|--------------|-----|------------------------|
| Manufacturing/Delivery | | | |
| Supply Chain | | | |
| Quality Systems | | | |
| Facilities | | | |
| Processes | | | |

### Technology Assessment
**Core Technology:**
[Assessment of technology/IP]

**IT Infrastructure:**
[Assessment of systems]

**Integration Considerations:**
[Key technology integration points]

### Talent Assessment
**Critical Employees:**
| Name | Role | Retention Risk | Retention Plan |
|------|------|----------------|----------------|
| | | | |

**Workforce Analysis:**
[Summary of workforce considerations]

### Culture Assessment
| Dimension | Target | Acquirer | Compatibility |
|-----------|--------|----------|---------------|
| Decision-making | | | |
| Risk tolerance | | | |
| Work pace | | | |
| Values | | | |

**Culture Integration Plan:**
[Summary of cultural integration approach]

### Customer & Revenue Quality
[Assessment of customer base and revenue quality]

### Integration Complexity
**Overall:** [Low/Medium/High/Very High]

### Lee's Operational View
"[Technocrat perspective]"

### Geneva's Culture View
"[Stakeholder mediator perspective]"

---

## 5. Risk Assessment

### Deal Risks
| Risk | Likelihood | Impact | Mitigation |
|------|------------|--------|------------|
| | H/M/L | H/M/L | |

### Integration Risks
| Risk | Likelihood | Impact | Mitigation |
|------|------------|--------|------------|
| | | | |

### Strategic & Market Risks
-

### Legal & Regulatory Risks
-

### Financial Risks
-

### Risk Priority Matrix

**Critical Risks (High x High):**
1. [Risk + mitigation]

**Major Risks:**
1. [Risk + mitigation]

**Watch List:**
1. [Risk + trigger]

### Deal Breaker Assessment
[Any conditions that would cause us to walk away]

### Burke's Risk View
"[Conservative perspective]"

---

## 6. Integration Planning

### Integration Philosophy
**Model:** [Full/Partial/Holding/Reverse]
**Rationale:** [Why this approach]

### Integration Governance
**IMO Leader:**
**Steering Committee:**
**Decision Rights:**

### Day 1 Readiness Checklist
- [ ] Legal close complete
- [ ] Leadership announced
- [ ] Employee communications ready
- [ ] Customer communications ready
- [ ] Systems access enabled
- [ ] Payroll/HR transitioned

### First 100 Days Plan
| Days | Focus | Key Deliverables |
|------|-------|------------------|
| 1-30 | Stabilization | |
| 31-60 | Assessment | |
| 61-100 | Execution | |

### Synergy Capture Roadmap
| Synergy | Year 1 | Year 2 | Full Run-Rate |
|---------|--------|--------|---------------|
| Cost | | | |
| Revenue | | | |
| **Total** | | | |

### Integration Risks & Mitigations
| Risk | Mitigation | Owner |
|------|------------|-------|
| | | |

### Resource Requirements
**Total Integration Investment:** $X

### Lee's Integration View
"[Operational perspective]"

---

## 7. Stakeholder & Communication Strategy

### Stakeholder Map
| Stakeholder | Key Concerns | Priority | Approach |
|-------------|--------------|----------|----------|
| | | | |

### Key Messages
**Core Narrative:**
[Summary of deal story]

**Audience-Specific:**
- Employees: [message]
- Customers: [message]
- Investors: [message]

### Communication Timeline
| Phase | Timing | Key Communications |
|-------|--------|-------------------|
| Announcement | | |
| Close | | |
| Integration | | |

### Key FAQs
| Question | Response |
|----------|----------|
| | |

### Scenario Response Plans
| Scenario | Response |
|----------|----------|
| | |

### Giuseppe's Communications View
"[Communications perspective]"

---

## 8. Deal Recommendation

### Recommendation: [PROCEED / PROCEED WITH CONDITIONS / DO NOT PROCEED]

**Rationale:**
[2-3 sentence summary of why]

### Deal Scorecard
| Dimension | Score (1-5) | Confidence |
|-----------|-------------|------------|
| Strategic fit | | |
| Financial attractiveness | | |
| Operational feasibility | | |
| Integration complexity | | |
| Risk profile | | |
| **Overall** | | |

### Advisor Perspectives
- **Sun (Strategy):** [summary]
- **Burke (Risk):** [summary]
- **Lee (Operations):** [summary]
- **Sophia (Ethics):** [summary]

### Conditions for Proceeding
1. [condition]
2. [condition]

### Key Risks to Monitor
| Risk | Trigger | Response |
|------|---------|----------|
| | | |

### Dissenting Views
[Alternative perspectives]

### Next Steps
| Action | Owner | Timeline |
|--------|-------|----------|
| | | |

---

## Appendix

### Document History
| Version | Date | Author | Changes |
|---------|------|--------|---------|
| 1.0 | {date} | M&A Due Diligence Workflow | Initial draft |

### Sources Consulted
-

### Due Diligence Team
-

### Methodology
This due diligence was conducted using the M&A Due Diligence workflow, incorporating perspectives from strategic, operational, financial, and risk advisors through structured multi-dimensional analysis.
