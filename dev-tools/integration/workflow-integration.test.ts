...
vi.mock('crypto', () => {
  ...
  const mockCrypto = {
    randomUUID: vi.fn(() => `integration-test-uuid-${++randomUUIDCounter}`),
    randomBytes: vi.fn(() => {
      // Return different bytes each time to ensure unique tokens
      const counter = ++randomBytesCounter;
...
describe('Workflow Integration Tests', () => {
  ...
  describe('Authentication to Authorization Workflow', () => {
    test('should complete full auth workflow from login to resource access', async () => {
      ...
      // Token is generated by TokenGenerator which produces bmad.v1.xxx format
      expect(authToken?.accessToken).toMatch(/^bmad\.v1\./);
      ...
      // The developer role has 'dev-tools:*' permission
      const canUseDevTools = await authManager.authorize(
        authToken!.accessToken,
        'dev-tools',
        'write'
      );
      expect(canUseDevTools).toBe(true);
  ...
  describe('Error Handling and Recovery', () => {
    test('should handle authentication failures gracefully', async () => {
      ...
      // Test with missing username - AuthManager validates this and returns null
