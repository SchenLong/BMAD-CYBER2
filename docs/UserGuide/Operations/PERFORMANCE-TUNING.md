...
2. [LLM Provider Optimization](#llm-provider-optimization)
3. [Token Management Optimization](#token-management-optimization)
4. [Workflow Caching](#workflow-caching)
5. [Validator Performance](#validator-performance)
6. [Resource Limits](#resource-limits)
7. [Audit Logging Optimization](#audit-logging-optimization)
...
// .claude/settings.json
{
  "hooks": {
    "pre-tool-use": [
      ...
      {"path": ".claude/validators/rate_limiter.py", "tools": ["*"]},
      {"path": ".claude/validators/recursion_guard.py", "tools": ["Skill"]},

      // Medium validators (10-50ms)
      {"path": ".claude/validators/token_validator.py", "tools": ["*"]},
      {"path": ".claude/validators/outside_repo_guard.py", "tools": ["Read", "Glob"]},

      // Slower validators last (> 50ms)
      {"path": ".claude/validators/jailbreak_guard.py", "tools": ["*"]},
      {"path": ".claude/validators/bash_safety.py", "tools": ["Bash"]}
    ]
  }
}
```
...
```json
{
  "hooks": {
    "pre-tool-use": [
      ...
      {"path": ".claude/validators/secret_guard.py", "tools": ["Write", "Edit"]},

      // Only for network operations
      {"path": ".claude/validators/plugin_permissions.py", "tools": ["WebFetch"]}
    ]
  }
}
```
...
# bash_safety.py         45ms   150ms  1000
# jailbreak_guard.py     120ms  500ms  1000  <-- Bottleneck
```
