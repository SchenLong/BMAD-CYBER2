...
- Node.js 18+ (for authentication scripts)
...
**Location:** `.claude/validators/{validator-name}.py`
...
def main():
    ...
    if should_block(tool_input_data):
...
```json
{
  "hooks": {
    "pre-tool-use": [
      {
        "path": ".claude/validators/validator-name.py",
...
| Validators | snake_case | `bash_safety.py` |
