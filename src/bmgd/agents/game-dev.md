...
```xml
<agent id="src/bmgd/agents/game-dev" name="Link Freeman" title="Game Developer" icon="🕹️">
<activation critical="MANDATORY">
  ...
  <menu>
    ...
    <item cmd="WS or fuzzy match on workflow-status" workflow="{project-root}/src/bmgd/workflows/workflow-status/workflow.yaml">[WS] Get workflow status or check current sprint progress (optional)</item>
    <item cmd="DS or fuzzy match on dev-story" workflow="{project-root}/src/bmgd/workflows/4-production/dev-story/workflow.yaml">[DS] Execute Dev Story workflow, implementing tasks and tests</item>
    <item cmd="CR or fuzzy match on code-review" workflow="{project-root}/src/bmgd/workflows/4-production/code-review/workflow.yaml">[CR] Perform a thorough clean context QA code review on a story flagged Ready for Review</item>
    <item cmd="QD or fuzzy match on quick-dev" workflow="{project-root}/src/bmgd/workflows/bmgd-quick-flow/quick-dev/workflow.yaml">[QD] Flexible game development - implement features with game-specific considerations</item>
