#!/usr/bin/env python3
"""
BMAD-CYBER2 Direct Prompt Injection Detection Engine (Fixed)
...
class PromptInjectionDetector:
    ...
    def _setup_logging(self) -> logging.Logger:
        ...
        # Create handler with proper file path
        import os
        log_dir = '/Users/paultinp/BMAD-CYBER2/.claude/logs'
        os.makedirs(log_dir, exist_ok=True)
        
        handler = logging.FileHandler(f'{log_dir}/security.log')
    ...
    def behavioral_analysis(self, text: str) -> Dict:
        ...
        risk_score = sum([
            int(indicators['excessive_capitalization']) * 0.2,
            int(indicators['multiple_exclamations']) * 0.1,
            int(indicators['command_structure']) * 0.3,
    ...
    def detect_injection(self, text: str, context: Optional[Dict] = None) -> DetectionResult:
        ...
        confidence_factors = [
            int(patterns_detected) * 0.4,
            int(high_entropy) * 0.3,
            behavioral_risk * 0.3,
        ]
        ...
        # Compile evidence (ensure JSON serializable)
        evidence = {
            'patterns_detected': matched_patterns,
            'entropy_score': float(entropy_score),
            'high_entropy': bool(high_entropy),
            'behavioral_indicators': {k: (int(v) if isinstance(v, bool) else v) for k, v in behavioral_indicators.items()},
            'text_length': len(text),
...
if __name__ == "__main__":
    ...
    print(f"\n✅ Testing complete! Generated {len(monitor.alert_queue)} security alerts.")
