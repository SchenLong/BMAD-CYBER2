...
class BMAdModulePackager {
    constructor(options = {}) {
        ...
        this.packageConfig = {
            ...
            author: 'BMAD Development Team <dev@bmad.ai>',
            maintainers: [
                {
    ...
    async validateModuleYaml(teamCode, teamPath) {
        ...
        try {
            ...
            if (moduleConfig.code !== teamCode) {
                this.validationResults.errors.push(`${teamCode}: module.yaml code '${moduleConfig.code}' doesn't match directory name '${teamCode}'`);
            }
        }
        catch (error) {
    ...
    async validateAgents(teamCode, teamPath, expectedCount) {
        ...
        try {
            ...
            if (mdFiles.length !== expectedCount) {
                this.validationResults.warnings.push(`${teamCode}: Expected ${expectedCount} agents, found ${mdFiles.length}`);
            }
        ...
        try {
            ...
            for (const section of requiredSections) {
                if (!content.includes(section)) {
                    this.validationResults.warnings.push(`${teamCode}: Agent ${path.basename(agentFilePath)} missing required section ${section}`);
                }
            }
        }
        catch (error) {
            this.validationResults.warnings.push(`${teamCode}: Agent ${path.basename(agentFilePath)} validation error - ${error.message}`);
        }
    }
    /**
    ...
    async validateWorkflows(teamCode, teamPath, expectedCount) {
        ...
        try {
            ...
            if (actualWorkflows.length !== expectedCount) {
                this.validationResults.warnings.push(`${teamCode}: Expected ${expectedCount} workflows, found ${actualWorkflows.length}`);
            }
        }
        catch (error) {
...
`;
    ...
    extractActivationSteps(xmlContent) {
        ...
        if (steps.length === 0) {
            ...
            steps.push({ number: 1, content: 'Load persona from agent configuration', critical: true }, { number: 2, content: 'Load and verify team configuration file', critical: true }, { number: 3, content: 'Display greeting and menu options', critical: true }, { number: 4, content: 'Wait for user input and process commands', critical: false });
        }
        return steps;
    }
    /**
...
To report a security vulnerability, please email security@bmad.ai with:
...
//# sourceMappingURL=bmad-specialized-teams-packager.js.map