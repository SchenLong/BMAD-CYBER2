...
class BMADPackageRegistryManager {
    ...
    async verifyNodeCompatibility(packageInfo) {
        ...
        try {
            ...
            result.nodeVersion.satisfied = semver.satisfies(this.runtimeEnv.node.version, result.nodeVersion.required);
            if (!result.nodeVersion.satisfied) {
                result.compatible = false;
                result.errors.push(`Node.js version ${result.nodeVersion.current} does not satisfy requirement ${result.nodeVersion.required}`);
            }
            // Check NPM version compatibility
            result.npmVersion.satisfied = semver.satisfies(this.runtimeEnv.npm.version, result.npmVersion.required);
            if (!result.npmVersion.satisfied) {
                result.compatible = false;
                result.errors.push(`NPM version ${result.npmVersion.current} does not satisfy requirement ${result.npmVersion.required}`);
            }
        ...
        try {
            ...
            for (const [depName, requiredVersion] of Object.entries(allDeps || {})) {
                ...
                if (!installedVersion) {
                    ...
                    try {
                        const satisfies = semver.satisfies(semver.clean(installedVersion) || installedVersion, requiredVersion);
                        if (!satisfies) {
...
//# sourceMappingURL=package-registry-manager.js.map