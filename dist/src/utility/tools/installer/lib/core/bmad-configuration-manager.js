...
class BMADConfigurationManager {
    ...
    async processConfiguration(config, context = {}) {
        ...
        try {
            ...
            const substitutedConfig = await this.performAdvancedSubstitution(environmentalConfig, resolvedContext);
            // Stage 4: Apply conditional configuration blocks
            const conditionalConfig = await this.applyConditionalConfigurations(substitutedConfig, resolvedContext);
            // Stage 5: Validate and finalize configuration
            const finalConfig = await this.finalizeConfiguration(conditionalConfig, resolvedContext);
...
//# sourceMappingURL=bmad-configuration-manager.js.map