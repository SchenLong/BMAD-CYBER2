    ...
    async installTeamModule(teamCode, options = {}) {
        ...
        try {
            ...
            await this.executeStep(session, 'installation_execution', async () => {
                ...
                const executionResult = await this.dependencyManager.executeInstallationPlan(installationPlan, executionOptions);
        ...
        for (const teamCode of teams) {
            try {
                ...
                if (result.success) {
                    ...
                    const processedResult = await this.configurationManager.processConfiguration(result.configuration, { teamCode, environment: options.environment || 'production' });
                    if (processedResult.success) {
                        result.configuration = processedResult.configuration;
                    }
                    // Validate the configuration
                    if (this.config.enableValidation) {
                        const validationResult = await this.configurationValidator.validateConfiguration(result.configuration, { teamCode });
...
//# sourceMappingURL=bmad-installation-orchestrator.js.map