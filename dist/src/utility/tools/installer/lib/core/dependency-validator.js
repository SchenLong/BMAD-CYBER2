...
class DependencyValidator {
    ...
    async validateCompatibility(moduleName, moduleInfo) {
        ...
        if (coreCompatibility) {
            const compatible = await this.checkCompatibilityEntry(moduleName, moduleInfo.version, 'bmad:core', this.coreVersion, coreCompatibility);
        ...
        if (this.installedModules) {
            for (const installedModule of this.installedModules) {
                ...
                if (compatibility) {
                    const compatible = await this.checkCompatibilityEntry(moduleName, moduleInfo.version, installedModule.name, installedModule.version, compatibility);
                    if (!compatible.compatible) {
                        result.errors.push({
    ...
    async resolveVersionConflict(conflict, moduleGraph) {
        ...
        const compatibleVersions = await this.findCompatibleVersions(conflict.dependencyModule, moduleGraph);
