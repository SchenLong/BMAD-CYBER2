...
class MultiModuleValidator {
    ...
    async validateSourceStructure() {
        ...
        for (const teamName of expectedTeams) {
            ...
            if (agentFiles.length !== expectedCounts[teamName].agents) {
                this.addResult('failed', `${teamName}: Expected ${expectedCounts[teamName].agents} agents, found ${agentFiles.length}`);
            }
            else {
            ...
            const workflowDirs = fs.existsSync(workflowsPath) ?
                fs.readdirSync(workflowsPath).filter(f => fs.statSync(path.join(workflowsPath, f)).isDirectory()) : [];
            if (workflowDirs.length !== expectedCounts[teamName].workflows) {
                this.addResult('failed', `${teamName}: Expected ${expectedCounts[teamName].workflows} workflows, found ${workflowDirs.length}`);
            }
            else {
    ...
    async validateAgentConversions() {
        ...
        for (const teamName of teams) {
            ...
            if (sourceAgents.length !== distAgents.length) {
                this.addResult('failed', `${teamName}: Agent conversion mismatch. Source: ${sourceAgents.length}, Converted: ${distAgents.length}`);
            }
            else {
    ...
    async validateWorkflowConversions() {
        ...
        for (const teamName of teams) {
            ...
            const sourceWorkflows = fs.readdirSync(sourceWorkflowsPath).filter(f => fs.statSync(path.join(sourceWorkflowsPath, f)).isDirectory());
            const distWorkflows = fs.readdirSync(distWorkflowsPath).filter(f => fs.statSync(path.join(distWorkflowsPath, f)).isDirectory());
            ...
            if (sourceWorkflows.length !== distWorkflows.length) {
                this.addResult('failed', `${teamName}: Workflow conversion mismatch. Source: ${sourceWorkflows.length}, Converted: ${distWorkflows.length}`);
            }
            else {
