...
class MultiModuleBuilder {
    ...
    async buildTeamModule(teamName) {
        ...
        if (fs.existsSync(workflowsPath)) {
            const workflowDirs = fs.readdirSync(workflowsPath).filter(f => fs.statSync(path.join(workflowsPath, f)).isDirectory());
    ...
    extractTeam(content) {
        ...
        if (content.includes('cybersec') || content.includes('security'))
            return 'cybersec-team';
        if (content.includes('intel') || content.includes('intelligence'))
            return 'intel-team';
        if (content.includes('legal') || content.includes('law'))
            return 'legal-team';
        if (content.includes('strategy') || content.includes('strategic'))
