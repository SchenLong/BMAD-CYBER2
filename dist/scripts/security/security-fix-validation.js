...
class SecurityFixValidation {
    ...
    async validatePrivilegeEscalationFix() {
        ...
        for (const attack of this.originalAttacks.privilegeEscalation) {
            totalAttempts++;
            const result = this.privilegeProtection.validatePrivilegeEscalation(attack.from, attack.to, 'invalidToken');
            if (!result.success) {
                blockedAttempts++;
    ...
    generateComparisonReport() {
        ...
        const totalOriginalRisk = (overall.privilegeEscalation.originalSuccessRate +
            overall.encodedPayload.originalBypassRate +
            overall.promptInjection.originalSuccessRate) / 3;
        const totalNewRisk = (overall.privilegeEscalation.newSuccessRate +
            overall.encodedPayload.newBypassRate +
            overall.promptInjection.newSuccessRate) / 3;
...
//# sourceMappingURL=security-fix-validation.js.map