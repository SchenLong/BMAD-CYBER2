...
class PerformanceRegressionDetector {
    ...
    async analyzeRegressions() {
        ...
        if (this.isRegression(previousBenchmark.metrics?.agentLoading?.averageLoadTime || 0, this.currentBenchmark.metrics.agentLoading.averageLoadTime, this.thresholds.loadTimeRegression)) {
            regressions.push({
                type: 'Agent Loading',
                severity: 'HIGH',
                ...
                changePercent: this.calculateChangePercent(previousBenchmark.metrics.agentLoading.averageLoadTime, this.currentBenchmark.metrics.agentLoading.averageLoadTime),
                impact: 'Agent initialization performance degraded'
            });
        }
        // Check memory regression
        if (this.isRegression(previousBenchmark.metrics?.memory?.memoryPerAgentKB || 0, this.currentBenchmark.metrics.memory.memoryPerAgentKB, this.thresholds.memoryRegression)) {
            regressions.push({
                type: 'Memory Usage',
                severity: 'MEDIUM',
                ...
                changePercent: this.calculateChangePercent(previousBenchmark.metrics.memory.memoryPerAgentKB, this.currentBenchmark.metrics.memory.memoryPerAgentKB),
                impact: 'Memory consumption per agent increased significantly'
            });
        }
        // Check throughput regression
        if (this.isThroughputRegression(previousBenchmark.metrics?.throughput?.operationsPerSecond || 0, this.currentBenchmark.metrics.throughput.operationsPerSecond, this.thresholds.throughputRegression)) {
            regressions.push({
                ...
                changePercent: this.calculateChangePercent(previousBenchmark.metrics.throughput.operationsPerSecond, this.currentBenchmark.metrics.throughput.operationsPerSecond),
                impact: 'System throughput decreased significantly'
            });
        }
        // Check integration latency regression
        if (this.isRegression(previousBenchmark.metrics?.integration?.averageLatency || 0, this.currentBenchmark.metrics.integration.averageLatency, this.thresholds.integrationLatencyRegression)) {
            regressions.push({
                ...
                changePercent: this.calculateChangePercent(previousBenchmark.metrics.integration.averageLatency, this.currentBenchmark.metrics.integration.averageLatency),
...
//# sourceMappingURL=bmad-performance-regression-detector.js.map