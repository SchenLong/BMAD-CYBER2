---
name: step-04-evidence
description: Evaluate available evidence
workflow_path: '{project-root}/_bmad-output/bmb-creations/legal-team/workflows/dispute-strategy'
nextStepFile: '{workflow_path}/steps/step-05-opposing.md'
---

# Step 4: Evidence Assessment

## STEP GOAL

Evaluate available evidence and identify evidentiary strengths and gaps.

## EXECUTION SEQUENCE

### 1. Evidence Inventory

Catalog all available evidence:

**Documentary Evidence:**
- Contracts and agreements
- Correspondence (emails, letters, texts)
- Financial records
- Business records
- Government filings
- Photos/Videos

**Testimonial Evidence:**
- Client testimony
- Witness availability
- Expert witness needs
- Opposing party admissions

**Physical Evidence:**
- Products or goods
- Property
- Physical items in dispute

**Electronic Evidence:**
- Digital communications
- Database records
- Metadata
- Social media content

### 2. Evidence Strength Analysis

For each key piece of evidence:

| Evidence | Type | Proves | Authenticity | Admissibility Risk |
|----------|------|--------|--------------|-------------------|
| [Description] | [Doc/Test/Phys] | [What it shows] | [High/Med/Low] | [Issues] |

### 3. Evidence Gap Analysis

Identify missing evidence:

**Critical Gaps:**
- Evidence needed to prove essential elements
- Missing documentation
- Unavailable witnesses

**Discovery Opportunities:**
- Documents to request from opposing party
- Interrogatories to serve
- Depositions to take
- Third-party subpoenas

### 4. Evidence Preservation

Address preservation concerns:

**Preservation Steps:**
- Litigation hold requirements
- Document collection
- Electronic data preservation
- Witness statements

**Spoliation Risks:**
- Evidence at risk of destruction
- Third-party controlled evidence
- Time-sensitive preservation needs

### 5. Update Output

```markdown
## 4. Evidence Assessment

### Evidence Inventory

**Documentary Evidence:**
| Document | Description | Proves | Status |
|----------|-------------|--------|--------|
| [Doc] | [Description] | [Relevance] | [Have/Need] |
...

**Testimonial Evidence:**
| Witness | Relationship | Can Testify To | Availability |
|---------|--------------|----------------|--------------|
| [Name] | [Connection] | [Topics] | [Status] |
...

### Evidence Strength Assessment
**Strong Evidence:**
- [List strongest evidence]

**Moderate Evidence:**
- [List supporting evidence]

**Weak/Challenged Evidence:**
- [List potentially challenged evidence]

### Evidence Gaps
**Critical Missing Evidence:**
1. [Gap and impact]
2. [Gap and impact]

**Discovery Targets:**
1. [What to request and why]
2. [What to request and why]

### Preservation Status
[Status of evidence preservation efforts]
```

Update: `stepsCompleted: [1, 2, 3, 4]`

### 6. Menu

**[C]** Continue to opposing party analysis | **[E]** More evidence discussion | **[Q]** Questions
