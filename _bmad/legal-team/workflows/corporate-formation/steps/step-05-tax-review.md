---
name: step-05-tax-review
description: Tax specialist reviews implications
workflow_path: '{project-root}/_bmad/legal-team/workflows/corporate-formation'
nextStepFile: '{workflow_path}/steps/step-06-documents.md'
---

# Step 5: Tax Implications Review

## STEP GOAL

Tax specialist (Tribute) analyzes the tax implications of the selected structure.

## EXECUTION SEQUENCE

### 1. Tribute Introduction

"I'm Tribute, the tax specialist. Let me analyze the tax implications of your [Entity Type] formation in [Jurisdiction]."

### 2. Tax Analysis by Jurisdiction

**USA Entity Tax Analysis:**
- Federal tax classification (default vs. election)
- State tax obligations
- Pass-through vs. corporate taxation
- Self-employment tax considerations
- Estimated tax requirements
- SALT deduction impacts

**Spain Entity Tax Analysis:**
- Impuesto sobre Sociedades (Corporate Tax)
- VAT/IVA obligations
- Withholding on dividends
- Tax treaties applicable
- Transfer pricing considerations
- Annual reporting obligations

**Estonia Entity Tax Analysis:**
- 0% retained earnings treatment
- Distribution taxation (20% or 14%)
- VAT registration thresholds
- CFC rule considerations
- Substance requirements
- Exit taxation

### 3. Owner Tax Impact

Analyze impact on owners:

**US Resident Owners:**
- CFC/PFIC considerations (foreign entities)
- GILTI implications
- Form 5471/8865 requirements
- FBAR/FATCA obligations

**EU Resident Owners:**
- CFC rules by residence country
- Dividend taxation
- Capital gains on disposal
- Wealth tax considerations

**Spain Resident Owners:**
- IRPF implications
- Modelo 720 requirements
- Exit tax considerations

### 4. Tax Optimization Notes

```markdown
### Tax Considerations

**Entity Level:**
- [Tax rate and treatment]
- [Deductible expenses]
- [Loss utilization]

**Owner Level:**
- [Distribution taxation]
- [Reporting requirements]
- [Treaty benefits]

**Optimization Opportunities:**
1. [Opportunity 1]
2. [Opportunity 2]

**Compliance Requirements:**
- [Key filing obligations]
- [Deadlines]
```

### 5. Update Output

```markdown
## 5. Tax Implications Review

*Analyzed by: Tribute (Tax Specialist)*

### Entity Taxation
**Tax Regime:** [Description]
**Effective Rate:** [Estimated]
**Key Features:**
- [Feature 1]
- [Feature 2]

### Owner Impact
**For [Owner Type]:**
- [Impact 1]
- [Impact 2]

### Reporting Obligations
[List key compliance requirements]

### Recommendations
[Tax-related suggestions]

**Note:** This analysis is for general guidance. Consult a qualified tax advisor for your specific situation.
```

Update: `stepsCompleted: [1, 2, 3, 4, 5]`

### 6. Menu

**[C]** Continue to document requirements | **[T]** More tax questions | **[Q]** General questions
