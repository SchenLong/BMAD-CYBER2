# Exec-Ops Party Mode Presets

**Purpose:** Pre-configured agent groupings for common executive decision scenarios. Use these presets when invoking Party Mode during workflow steps.

---

## How to Use Presets

When at a MENU step and selecting **[P] Party Mode**, invoke a preset by name:

```
"Use the Strategic Council preset"
"Let's run the Ethics Review preset"
"Invoke the Power Analysis preset for this stakeholder map"
```

Or specify individual agents from any preset:

```
"From the Ethics Review preset, bring in Sophia and Jean-Luc"
```

---

## Preset Definitions

### 1. Strategic Council (Full Archetypes)

**ID:** `strategic-council`
**Purpose:** Comprehensive multi-perspective analysis for major strategic decisions
**Best For:** High-stakes decisions, transformational initiatives, board-level strategy

**Agents (8):**
| Icon | Name | Role in Council |
|------|------|-----------------|
| 🦊 | Niccolo | Power dynamics, realpolitik analysis |
| 🕊️ | Charles | Moral authority, unity building |
| ✊ | Maximilien | Systemic critique, bold alternatives |
| 🏛️ | Burke | Tradition, unintended consequences |
| ⚙️ | Lee | Efficiency, metrics, systems |
| ⚔️ | Musashi | Timing, decisive action |
| 🐉 | Sun | Grand strategy, positioning |
| 🖖 | Jean-Luc | Principled leadership |

**Discussion Format:**
Each archetype offers 1-2 paragraphs on the topic, with explicit bias acknowledgment. Expect tensions between:
- Niccolo vs Charles (realism vs idealism)
- Maximilien vs Burke (revolution vs tradition)
- Lee vs Jean-Luc (efficiency vs principles)

---

### 2. Ethics Review Board

**ID:** `ethics-review`
**Purpose:** Deep ethical analysis and values alignment
**Best For:** Decisions with moral dimensions, stakeholder impact, reputation risk

**Agents (3):**
| Icon | Name | Role in Review |
|------|------|----------------|
| ⚖️ | Sophia | Primary ethical framework analysis |
| 🖖 | Jean-Luc | Principled leadership perspective |
| 🕊️ | Charles | Moral authority and justice framing |

**Discussion Format:**
Sophia leads with ethical framework application (utilitarian, deontological, virtue, care ethics). Jean-Luc challenges with leadership principles. Charles frames in terms of higher purpose and unity.

**Typical Questions:**
- What values are in tension?
- Who bears the burden of this decision?
- What precedent does this set?
- Can we look back with pride?

---

### 3. Power Analysis Council

**ID:** `power-analysis`
**Purpose:** Political landscape mapping and influence strategy
**Best For:** Stakeholder mapping, coalition building, competitive positioning

**Agents (3):**
| Icon | Name | Role in Analysis |
|------|------|------------------|
| ♟️ | Magnus | Coalition math, political pathways |
| 🦊 | Niccolo | Hidden agendas, real motivations |
| 🐉 | Sun | Strategic positioning, terrain |

**Discussion Format:**
Magnus maps the political landscape and paths to victory. Niccolo exposes the uncomfortable truths about power and motivation. Sun provides the grand strategic view of positioning and timing.

**Typical Questions:**
- Who has real power vs nominal power?
- What do they actually want?
- Where's the path to majority support?
- What terrain advantages can we create?

---

### 4. Communication Strategy Team

**ID:** `comms-strategy`
**Purpose:** Message development, narrative framing, persuasion
**Best For:** Announcements, crisis communications, stakeholder messaging

**Agents (3):**
| Icon | Name | Role in Strategy |
|------|------|------------------|
| 📢 | Joseph | Narrative crafting, media strategy |
| 🎭 | Cicero | Argument construction, rhetoric |
| 🤝 | Geneva | Audience empathy, stakeholder concerns |

**Discussion Format:**
Joseph develops the core narrative and key messages. Cicero stress-tests arguments and strengthens persuasive elements. Geneva ensures messaging resonates with diverse stakeholder concerns.

**Typical Questions:**
- What's the headline we want?
- Where are the rhetorical weak points?
- How will each audience receive this?
- What questions should we anticipate?

---

### 5. Risk Assessment Panel

**ID:** `risk-assessment`
**Purpose:** Comprehensive risk identification and mitigation planning
**Best For:** Major initiatives, policy changes, strategic shifts

**Agents (3):**
| Icon | Name | Role in Assessment |
|------|------|-------------------|
| 🏛️ | Burke | Unintended consequences, what could go wrong |
| ⚙️ | Lee | Systems analysis, operational risks |
| 📊 | Augustus | Evidence-based risk quantification |

**Discussion Format:**
Burke raises concerns about disruption and unforeseen effects. Lee applies systematic risk analysis and operational metrics. Augustus grounds the discussion in evidence and historical precedent data.

**Typical Questions:**
- What unintended consequences might emerge?
- What does history tell us about similar changes?
- What are the operational failure modes?
- How confident are we in our assumptions?

---

### 6. Change Management Council

**ID:** `change-management`
**Purpose:** Balancing transformation with stability
**Best For:** Organizational change, policy reform, digital transformation

**Agents (3):**
| Icon | Name | Role in Council |
|------|------|-----------------|
| 🕊️ | Charles | Transformation vision, moral imperative |
| 🏛️ | Burke | Prudent reform, institutional wisdom |
| ⚙️ | Lee | Implementation efficiency, capability building |

**Discussion Format:**
Charles articulates the case for change and higher purpose. Burke ensures wisdom about what should be preserved. Lee focuses on practical implementation and measurable outcomes.

**Key Tension:** This preset intentionally creates productive tension between urgency and caution.

---

### 7. Negotiation Preparation

**ID:** `negotiation-prep`
**Purpose:** Prepare for critical negotiations with thorough analysis
**Best For:** Contract negotiations, stakeholder discussions, conflict resolution

**Agents (3):**
| Icon | Name | Role in Prep |
|------|------|--------------|
| 🤝 | Geneva | Interest mapping, BATNA analysis |
| ♟️ | Magnus | Power dynamics, leverage points |
| 🎭 | Cicero | Argument framing, objection handling |

**Discussion Format:**
Geneva maps underlying interests and identifies zones of possible agreement. Magnus analyzes power dynamics and leverage. Cicero prepares the persuasive framework and anticipates objections.

**Output Focus:**
- Interest vs position mapping
- BATNA analysis for all parties
- Key leverage points
- Prepared responses to likely objections

---

### 8. Crisis Response Team

**ID:** `crisis-response`
**Purpose:** Rapid crisis assessment and response planning
**Best For:** PR crises, operational failures, external threats

**Agents (4):**
| Icon | Name | Role in Response |
|------|------|------------------|
| 📢 | Joseph | Crisis communications lead |
| ♟️ | Magnus | Stakeholder management |
| 🖖 | Jean-Luc | Principled decision-making |
| ⚔️ | Musashi | Timing and decisive action |

**Discussion Format:**
Joseph frames the communications response. Magnus manages stakeholder dynamics. Jean-Luc ensures decisions remain principled under pressure. Musashi advises on timing and when to act decisively.

**Crisis Protocol:**
1. Assess: What do we know? What don't we know?
2. Stakeholders: Who needs to hear from us first?
3. Messaging: What do we say and when?
4. Action: What do we do and when?

---

### 9. Board Presentation Prep

**ID:** `board-prep`
**Purpose:** Prepare for board-level presentations with rigor
**Best For:** Board meetings, investor presentations, executive briefings

**Agents (3):**
| Icon | Name | Role in Prep |
|------|------|--------------|
| 📊 | Augustus | Evidence synthesis, data rigor |
| 📢 | Joseph | Narrative structure, key messages |
| 🎭 | Cicero | Q&A preparation, argument defense |

**Discussion Format:**
Augustus ensures data is rigorous and defensible. Joseph structures the narrative for maximum impact. Cicero stress-tests arguments and prepares for tough questions.

**Output Focus:**
- Executive summary with evidence
- Narrative arc for presentation
- Anticipated questions with prepared answers
- Risk acknowledgment with mitigation

---

### 10. Strategic Planning Session

**ID:** `strategic-planning`
**Purpose:** Long-term strategic thinking with diverse philosophies
**Best For:** Annual planning, strategic reviews, vision setting

**Agents (4):**
| Icon | Name | Role in Planning |
|------|------|------------------|
| 🐉 | Sun | Grand strategy, positioning |
| ⚙️ | Lee | Capability building, execution |
| 🏛️ | Burke | Sustainability, what to preserve |
| ✊ | Maximilien | Bold vision, disruption opportunities |

**Discussion Format:**
Sun provides the strategic vision and positioning. Lee focuses on practical capability development. Burke ensures strategic continuity. Maximilien challenges the team to think bolder.

**Key Questions:**
- Where should we position ourselves?
- What capabilities must we build?
- What should we preserve vs change?
- What bold moves are we afraid to consider?

---

### 11. Conflict Resolution

**ID:** `conflict-resolution`
**Purpose:** Navigate interpersonal or organizational conflicts toward constructive resolution
**Best For:** Internal disputes, team conflicts, stakeholder disagreements, relationship repair

**Agents (4):**
| Icon | Name | Role in Resolution |
|------|------|-------------------|
| 🤝 | Geneva | Lead mediator, interest mapping, finding common ground |
| 🖖 | Jean-Luc | Principled facilitation, ensuring dignity for all parties |
| 🕊️ | Charles | Unity building, moral framing, healing divisions |
| ⚖️ | Sophia | Values alignment, fairness assessment, ethical mediation |

**Discussion Format:**
Geneva leads with interest-based mediation, separating positions from underlying needs. Jean-Luc ensures all parties are treated with dignity and decisions align with principles. Charles frames the path toward reconciliation and unity. Sophia monitors for fairness and values alignment.

**Resolution Protocol:**
1. **Understand:** What does each party actually need (not just want)?
2. **Acknowledge:** What legitimate grievances exist on each side?
3. **Find:** Where do interests actually align?
4. **Create:** What options expand the pie rather than divide it?
5. **Commit:** What agreements can all parties own?

**Key Questions:**
- What are the underlying interests behind each position?
- What would "winning" actually look like for each party?
- What shared values or goals can unite the parties?
- How do we create space for face-saving on all sides?

---

### 12. All-Out War

**ID:** `all-out-war`
**Purpose:** Maximum competitive intensity for existential threats or winner-take-all situations
**Best For:** Hostile takeovers, competitive survival, proxy fights, market battles, adversarial negotiations

**Agents (5):**
| Icon | Name | Role in War Council |
|------|------|---------------------|
| 🦊 | Niccolo | Ruthless realpolitik, exploit every weakness |
| 🐉 | Sun | Grand strategy, terrain, deception, positioning |
| ⚔️ | Musashi | Timing, decisive strikes, economy of action |
| ♟️ | Magnus | Coalition warfare, political maneuvering |
| 📢 | Joseph | Information warfare, narrative control |

**Discussion Format:**
This is the most aggressive preset - no diplomatic niceties, pure competitive strategy. Niccolo identifies vulnerabilities and leverage. Sun provides strategic architecture. Musashi advises on timing and decisive action. Magnus handles coalition politics. Joseph wages information warfare.

**War Doctrine:**
1. **Know your enemy:** What are their weaknesses, constraints, and pressure points?
2. **Control terrain:** Where do we fight? What advantages can we create?
3. **Timing:** When do we strike? When do we wait?
4. **Deception:** What does the enemy believe vs. what is true?
5. **Coalitions:** Who can we bring to our side? Who can we neutralize?
6. **Victory conditions:** How do we know when we've won?

**Rules of Engagement:**
- Assume adversary is rational and capable
- Map all stakeholders as allies, enemies, or swing votes
- Identify and protect our vulnerabilities
- Prepare for escalation and counter-moves
- Define acceptable costs and walk-away points

**Warning:** This preset is intentionally aggressive. Use only for genuinely adversarial situations. Consider ethics implications before deployment.

---

### 13. Corporate Political Game

**ID:** `corporate-politics`
**Purpose:** Navigate complex internal politics, power dynamics, and organizational maneuvering
**Best For:** Promotion campaigns, budget battles, turf wars, reorganizations, succession planning

**Agents (5):**
| Icon | Name | Role in Political Strategy |
|------|------|---------------------------|
| ♟️ | Magnus | Primary strategist, coalition math, power mapping |
| 🦊 | Niccolo | Hidden agendas, real motivations, cynical truth-telling |
| 🎭 | Cicero | Persuasion, framing, winning arguments |
| 🤝 | Geneva | Building alliances, managing relationships |
| 📢 | Joseph | Reputation management, narrative positioning |

**Discussion Format:**
Magnus leads strategic analysis of the political landscape - who has power, who wants what, and how to build a winning coalition. Niccolo provides unvarnished truth about motivations and hidden agendas. Cicero prepares persuasive arguments and handles objections. Geneva advises on relationship building and alliance maintenance. Joseph manages reputation and narrative positioning.

**Political Playbook:**
1. **Map the terrain:**
   - Who has formal power? (titles, budgets, decision rights)
   - Who has informal power? (relationships, information, expertise)
   - Whose star is rising? Falling?

2. **Understand motivations:**
   - What does each player actually want?
   - What are they afraid of?
   - What would they trade?

3. **Build your coalition:**
   - Who are natural allies?
   - Who can be converted?
   - Who must be neutralized?

4. **Manage your brand:**
   - How are you perceived?
   - What narrative serves your goals?
   - Who are your advocates?

5. **Execute with timing:**
   - When is the right moment to move?
   - What triggers should you wait for?
   - How do you maintain plausible deniability?

**Political Survival Rules:**
- Never make enemies unnecessarily
- Accumulate chits before you need them
- Let others take credit (when strategically valuable)
- Control information flow
- Build relationships before you need them
- Know when to fight and when to accommodate

**Ethical Note:** Corporate politics are reality. This preset helps navigate them effectively while maintaining integrity. The goal is influence through competence and relationships, not manipulation.

---

## Custom Preset Creation

You can create ad-hoc presets during any Party Mode session:

```
"Create a custom preset with Augustus, Geneva, and Sun for this specific analysis"
```

Or combine elements from existing presets:

```
"Use the Ethics Review preset but add Magnus for the political dimension"
```

---

## Preset Quick Reference

| Preset ID | Agents | Use Case |
|-----------|--------|----------|
| `strategic-council` | All 8 Archetypes | Major strategic decisions |
| `ethics-review` | Sophia, Jean-Luc, Charles | Moral dimensions |
| `power-analysis` | Magnus, Niccolo, Sun | Political landscape |
| `comms-strategy` | Joseph, Cicero, Geneva | Messaging, announcements |
| `risk-assessment` | Burke, Lee, Augustus | Risk identification |
| `change-management` | Charles, Burke, Lee | Transformation balance |
| `negotiation-prep` | Geneva, Magnus, Cicero | Negotiation readiness |
| `crisis-response` | Joseph, Magnus, Jean-Luc, Musashi | Crisis situations |
| `board-prep` | Augustus, Joseph, Cicero | Board presentations |
| `strategic-planning` | Sun, Lee, Burke, Maximilien | Long-term strategy |
| `conflict-resolution` | Geneva, Jean-Luc, Charles, Sophia | Internal conflicts, disputes |
| `all-out-war` | Niccolo, Sun, Musashi, Magnus, Joseph | Competitive warfare |
| `corporate-politics` | Magnus, Niccolo, Cicero, Geneva, Joseph | Internal politics |

---

## Integration with Workflows

Each exec-ops workflow has suggested presets at key steps:

| Workflow | Suggested Presets |
|----------|-------------------|
| strategic-decision-workshop | `strategic-council`, `ethics-review` |
| stakeholder-negotiation-prep | `negotiation-prep`, `power-analysis` |
| board-presentation-prep | `board-prep`, `risk-assessment` |
| crisis-response-planning | `crisis-response`, `comms-strategy` |
| strategic-planning-session | `strategic-planning`, `change-management` |
| policy-development | `ethics-review`, `risk-assessment` |
