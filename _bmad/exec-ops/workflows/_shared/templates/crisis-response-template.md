---
stepsCompleted: []
createdDate: {date}
workflow: crisis-response-planning
status: draft
crisisTitle: ""
severity: ""
---

# Crisis Response Plan: {title}

## Crisis Summary

**Crisis Type:**
**Severity Level:** Low / Medium / High / Critical
**Date Identified:**
**Status:** Active / Contained / Resolved

[Brief description of the crisis]

---

## 1. Situation Assessment

### What Happened
[Factual account of the crisis]

### Impact Assessment
| Dimension | Impact | Severity |
|-----------|--------|----------|
| Financial | | |
| Reputational | | |
| Operational | | |
| Legal/Regulatory | | |
| Employee | | |
| Customer | | |

### Stakeholders Affected
| Stakeholder | How Affected | Urgency |
|-------------|--------------|---------|
| | | Immediate/24h/48h/Week |

### What We Know vs Don't Know
**Confirmed facts:**
-

**Unconfirmed/Investigating:**
-

**Unknown:**
-

---

## 2. Immediate Actions (First 24-48 Hours)

### Hour 0-4
| Action | Owner | Status |
|--------|-------|--------|
| Activate crisis team | | |
| Secure evidence/data | | |
| Initial stakeholder notification | | |
| Assess containment options | | |

### Hour 4-24
| Action | Owner | Status |
|--------|-------|--------|
| | | |

### Hour 24-48
| Action | Owner | Status |
|--------|-------|--------|

### Do NOT Do
- [ ]
- [ ]

---

## 3. Crisis Team

### Core Team
| Role | Name | Contact | Backup |
|------|------|---------|--------|
| Crisis Lead | | | |
| Communications | | | |
| Legal | | | |
| Operations | | | |
| Technical/IT | | | |

### Extended Team (As Needed)
| Role | Name | When to Activate |
|------|------|------------------|
| | | |

### Decision Authority
**Can decide immediately:**
**Needs executive approval:**
**Needs board notification:**

---

## 4. Stakeholder Communications (Joseph's Plan)

### Internal Communications

**Employees**
- **Timing:**
- **Channel:**
- **Message:**
- **Messenger:**
- **Q&A prepared:** Yes/No

**Leadership/Board**
- **Timing:**
- **Channel:**
- **Message:**
- **Key data points:**

### External Communications

**Customers/Clients**
- **Timing:**
- **Channel:**
- **Message:**
- **Messenger:**

**Media**
- **Proactive or reactive:**
- **Spokesperson:**
- **Key messages:**
  1.
  2.
  3.
- **Holding statement:**

**Regulators/Authorities**
- **Required notifications:**
- **Timeline:**
- **Contact:**

**Partners/Vendors**
- **Who needs to know:**
- **Message:**

---

## 5. Media Strategy

### Press Statement (Draft)
```
[Organization] is aware of [situation]. We are [taking action].

[Key message about values/commitment]

[What stakeholders can expect]

[Contact information]
```

### Q&A for Spokesperson

| Question | Response | If Pushed |
|----------|----------|-----------|
| What happened? | | |
| Who is affected? | | |
| What are you doing? | | |
| Who is responsible? | | |
| When will it be resolved? | | |

### No-Go Zones
**Do NOT say:**
-
-

**Do NOT speculate about:**
-

### If Asked About [Sensitive Topic]
**Bridge to:**

---

## 6. Political Dimension (Magnus's Analysis)

### Power Dynamics
**Who gains from this crisis:**
**Who loses:**
**Who can help us:**
**Who might exploit this:**

### Regulatory/Government Implications
-

### Competitor Response Likely
-

### Coalition/Support Building
**Allies to activate:**
**Stakeholders to neutralize:**

---

## 7. Principled Response (Jean-Luc's Guidance)

### Values to Uphold
-

### Lines We Will Not Cross
-

### How to Maintain Dignity
**For affected parties:**
**For our organization:**

### Long-term Reputation Considerations
-

---

## 8. Scenario Planning

### Best Case
**Outcome:**
**Timeline:**
**Required actions:**

### Likely Case
**Outcome:**
**Timeline:**
**Required actions:**

### Worst Case
**Outcome:**
**Timeline:**
**Required actions:**

### Black Swan (Unexpected Escalation)
**If [X] happens:**
**Response:**

---

## 9. Recovery Path

### Short-term (1-4 weeks)
| Milestone | Target Date | Owner |
|-----------|-------------|-------|
| | | |

### Medium-term (1-3 months)
| Milestone | Target Date | Owner |
|-----------|-------------|-------|
| | | |

### Long-term Reputation Recovery
**Narrative we want in 6 months:**
**Actions to get there:**

### Lessons Learned Process
**Post-mortem scheduled:**
**Participants:**
**Output:**

---

## 10. Monitoring & Escalation

### Key Metrics to Track
| Metric | Threshold | Action if Exceeded |
|--------|-----------|-------------------|
| Media mentions | | |
| Social sentiment | | |
| Customer complaints | | |
| | | |

### Escalation Triggers
**Escalate to [Level] if:**
-

### Check-in Schedule
| Frequency | Participants | Focus |
|-----------|--------------|-------|
| Every 4 hours | Crisis team | Status, actions |
| Daily | Leadership | Progress, decisions |
| Weekly | Board | Recovery, lessons |

---

## 11. Documentation

### Evidence Preservation
- [ ] Logs secured
- [ ] Communications archived
- [ ] Timeline documented
- [ ] Witness statements

### Decision Log
| Date/Time | Decision | Rationale | Who Decided |
|-----------|----------|-----------|-------------|
| | | | |

### Action Log
| Date/Time | Action | Owner | Outcome |
|-----------|--------|-------|---------|
| | | | |

---

## Appendix

### Contact List
[Emergency contacts, external advisors, etc.]

### Templates
[Pre-approved message templates]

### Regulatory Requirements
[Notification requirements, timelines]

### Methodology
This crisis response plan was developed using the Crisis Response Planning workflow with input from Joseph (communications), Magnus (political dynamics), Geneva (stakeholder management), and Jean-Luc (principled leadership).
