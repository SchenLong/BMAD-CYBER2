...
run_automated_tests() {
    ...
    if [[ -f "${PROJECT_ROOT}/security-testing-framework/bmad-enterprise-security-tester.js" ]]; then
        info "Executing enterprise security testing framework..."
        
        if node "${PROJECT_ROOT}/security-testing-framework/bmad-enterprise-security-tester.js" 2>&1 | tee -a "$LOG_FILE"; then
